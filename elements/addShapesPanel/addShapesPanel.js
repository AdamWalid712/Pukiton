...
const btnClasses = "size-10 text-4xl text-black flex items-center justify-center";

// Main Panel Setup
addShapesPanel.id = "addShapesPanel";
addShapesPanel.className = `${glass} rounded-3xl w-48 h-max px-2 py-0 m-0 fixed bottom-16 left-1/2 -translate-x-1/2 flex flex-col items-center divide-y-2 divide-black hidden text-black`;

// Exit Button Setup
exitBtn.className = "ri-close-line text-black absolute top-2 right-2 size-8 flex items-center justify-center text-2xl";
...
fillShapesContainer.innerHTML = `
    <h2 class="text-black text-sm font-bold mb-1">fill shapes</h2>
...
strokeShapesContainer.innerHTML = `
    <h2 class="text-black text-sm font-bold mb-1">stroke shapes</h2>
    <div class="w-full flex justify-center gap-2">
...
otherShapesContainer.innerHTML = `
    <h2 class="text-black text-sm font-bold mb-1">other</h2>
    <div class="w-full flex justify-center gap-2 items-center">
        ...
        <div class="inline-block p-0 m-0">
            ...
            <label for="add-image-inp" class="${btnClasses} ri-image-add-fill cursor-pointer"></label>
        </div>
        <button class="size-10 text-2xl text-black ri-subtract-line flex items-center justify-center" onclick="addLine()" type="button"></button>
    </div>
`;
